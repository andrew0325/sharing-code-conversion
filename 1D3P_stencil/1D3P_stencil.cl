...
#define shr_sz (local_sz + (max3(idx_ofst_x1, idx_ofst_x2, idx_ofst_x3)))
...
__kernel void stencil_sharing(__global int *A, __global int *B)
{
	...
	if(get_local_size(0) > ofst_diff_max){
		...
		for(int i = 0; i < 100000000; i++){
			B[g_xid] =  A[1 * g_xid + idx_ofst_x1] 
				+ A[1 * g_xid + idx_ofst_x2]
				+ A[1 * g_xid + idx_ofst_x3];
		}
	}
}
